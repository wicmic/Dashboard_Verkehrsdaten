...
"""
#Verkehrsdaten
folder_path = 'flow'  #Ordnerpfad allenfalls anpassen
data_frames = []
...
#Gecleanter Datensatz lokal zwischenspeichern
df.to_csv("df_clean.csv", index=False, encoding="utf-8")
"""

#Einlesen des gecleanten Datensatzes für schnelleres Laden - Einige Datentypen müssen nach dem Import wieder konvertiert werden
df = pd.read_csv("df_clean.csv")
df['Datum'] = pd.to_datetime(df['Datum'])
df['Messstation'] = df['Messstation'].astype('string')
...
def balken_linien_diagramm(filtered_df):
    ...
    fig.update_layout(
        ...
        xaxis=dict(
            type='category', #Mit "category" anstelle von "date" werden die Daten kategorial interpretiert - d.h. die x-Werte werden nicht mehr als "Datetime"-Typ behandelt
            tickangle=-45,
            gridcolor='lightgray',
            zerolinecolor='black'),
    ...
    fig.update_xaxes(
        rangebreaks=[
            dict(bounds=["mon", "fri"])  # hide work week
        ]
    )
    """
    return fig
    ...
    dcc.Loading(
        ...
        children=[
            ...
            dbc.Row([
                ...
                dbc.Col([
                    dcc.Graph(id='grupp_balken_fig', style={'height': '500px', 'margin-top': '0px'}),
                ], width=5),
